...
const StockOut: React.FC = () => {
  ...
  const handleEditEntry = (entry: StockOutEntry) => {
    ...
    setFormData({
      ...
      dueDate: entry.dueDate ? (entry.dueDate instanceof Date ? entry.dueDate.toISOString().split('T')[0] : new Date(entry.dueDate).toISOString().split('T')[0]) : '',
      notes: entry.notes || ''
    });
    setShowForm(true);
  };
      ...
      {showForm && (
        <div className="card p-6">
          <h3 className="text-lg font-semibold text-gray-900 mb-4">
            {editingEntry ? 'Edit Sale Record' : 'Record New Sale'}
          </h3>
