...
const GeneralDebts: React.FC = () => {
  ...
  const handleEditDebt = (debt: GeneralDebt) => {
    ...
    setNewDebt({
      ...
      dueDate: debt.dueDate ? (debt.dueDate instanceof Date ? debt.dueDate.toISOString().split('T')[0] : new Date(debt.dueDate).toISOString().split('T')[0]) : '',
      issueDate: debt.issueDate instanceof Date ? debt.issueDate.toISOString().split('T')[0] : new Date(debt.issueDate).toISOString().split('T')[0],
