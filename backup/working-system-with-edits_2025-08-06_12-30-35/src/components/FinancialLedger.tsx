...
const FinancialLedger: React.FC = () => {
  ...
  // Scroll position preservation state
  const [preserveScrollPosition, setPreserveScrollPosition] = useState<number | null>(null);

  // Debug: Log modal state changes
  useEffect(() => {
    console.log('Report modal state changed:', showReportModal);
  }, [showReportModal]);

  // Effect to restore scroll position after transaction changes
  useEffect(() => {
    if (preserveScrollPosition !== null) {
      const restoreScroll = () => {
        window.scrollTo(0, preserveScrollPosition);
        console.log('Effect restored scroll position to:', preserveScrollPosition);
  ...
  }, [transactions, preserveScrollPosition]);
  ...
  const handleDeleteTransaction = async (id: string, description: string) => {
    if (window.confirm(`Are you sure you want to delete this transaction: "${description}"? This action cannot be undone.`)) {
      try {
        // Store current scroll position with multiple methods
        const scrollPosition = window.pageYOffset || document.documentElement.scrollTop;
        console.log('Stored scroll position:', scrollPosition);
        
        // Immediately prevent any default scrolling behavior
        const originalScrollBehavior = document.documentElement.style.scrollBehavior;
        document.documentElement.style.scrollBehavior = 'auto';
        
        await apiService.deleteTransaction(id);
        
        // Update state and immediately restore scroll
        setTransactions(prev => {
          ...
          setTimeout(() => {
            ...
            console.log('Immediately restored scroll to:', scrollPosition);
          }, 0);
          
          // Additional restoration attempts
          setTimeout(() => window.scrollTo({ top: scrollPosition, behavior: 'auto' }), 10);
          setTimeout(() => window.scrollTo({ top: scrollPosition, behavior: 'auto' }), 50);
          setTimeout(() => window.scrollTo({ top: scrollPosition, behavior: 'auto' }), 100);
      ...
      {showReportModal && (
        <div style={{
          ...
          <div style={{
            ...
            <p style={{ marginBottom: '20px', color: '#10b981', fontWeight: '500' }}>
              ✅ Modal is now working! You can see this form.
            </p>
